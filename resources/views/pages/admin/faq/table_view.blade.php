...
<p class="float-right"> Displaying {{$faq->firstItem()}}-{{ $faq->lastItem() }} of {{$faq->total()}}  Records</p>
@else
<p class="float-right"> Displaying {{' 0 '}}-{{ ' 0 ' }} of {{$faq->total()}}  Records</p>
@endif
...
<div class="table-responsive">
        ...
        @if(!$faq->isEmpty())
            ...
            <tr id="{{en_de_crypt($list->id,'e')}}">        
                ...
                <td class="actions" data-th="">
                  <a href="{{ url('/admin/faq/edit/' . en_de_crypt($list->id,'e') ) }}"><button class="btn badge-primary btn-xs"><i class="mdi mdi-lead-pencil"></i></button></a>
                </td>
            </tr>
           @endforeach
        @else
  ...
  {!! $faq->onEachSide(1)->links('ajax_pagination') !!}
</div>