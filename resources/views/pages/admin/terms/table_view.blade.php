...
<p class="float-right"> Displaying {{$terms->firstItem()}}-{{ $terms->lastItem() }} of {{$terms->total()}}  Records</p>
@else
<p class="float-right"> Displaying {{' 0 '}}-{{ ' 0 ' }} of {{$terms->total()}}  Records</p>
@endif
...
<div class="table-responsive">
        ...
        @if(!$terms->isEmpty())
            ...
            <tr id="{{en_de_crypt($list->id,'e')}}">        
                ...
                <td class="actions" data-th="">
                  <a href="{{ url('/admin/terms/edit/' . en_de_crypt($list->id,'e') ) }}"><button class="btn badge-primary btn-xs"><i class="mdi mdi-lead-pencil"></i></button></a>
                </td>
            </tr>
           @endforeach
        @else
  ...
  {!! $terms->onEachSide(1)->links('ajax_pagination') !!}
</div>