...
<p class="float-right"> Displaying {{$ticket->firstItem()}}-{{ $ticket->lastItem() }} of {{$ticket->total()}}  Records</p>
@else
<p class="float-right"> Displaying {{' 0 '}}-{{ ' 0 ' }} of {{$ticket->total()}}  Records</p>
@endif
...
<div class="table-responsive">
    ...
    <tbody>
        @if(!$ticket->isEmpty())
            ...
            <tr id="{{en_de_crypt($list->id,'e')}}">
                ...
                <td class="text-capitalize">{{$list->ticket_name}}</td>
                <td class="text-capitalize">{{$list->fee}}</td>
                <td class="text-capitalize">{{$list->created_at}}</td>
                <td class="actions" data-th="">
                  <a href="{{ url('/admin/ticket/edit/' . en_de_crypt($list->id,'e') ) }}"><button class="btn badge-primary btn-xs"><i class="mdi mdi-lead-pencil"></i></button></a>
                </td>
            </tr>
           @endforeach
        @else
  ...
  {!! $ticket->onEachSide(1)->links('ajax_pagination') !!}
</div>
