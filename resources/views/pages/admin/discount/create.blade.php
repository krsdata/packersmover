...
<div class="row">
  <div class="col-12 grid-margin">
    <div class="card">
      <div class="card-body">
        <h4 class="cust-card-title"><i class="mdi mdi-percent cust-box-icon"></i> @if(isset($discount)) {{'Update Discount'}} @else {{'Add New Discount'}} @endif</h4>
          <form  name="stock_frm" id="stock_frm" class="form-sample" method="POST" action = "{{ url('/admin/discount/store') }}"  accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                  {{ csrf_field() }}
          <input type="hidden" name="id" id="id" value="@if(isset($discount)){{en_de_crypt($discount->id,'e')}}@endif">
          {{-- <p class="card-description">
              @if(isset($discount)) {{'Update Discount info'}} @else {{' Add Discount info'}} @endif
          </p> --}}
          <div class="col-sm-12">&nbsp;
          </div>
          <div class="row">
            <div class="col-md-6">
              <div class="form-group row">
                <label class="col-sm-3 col-form-label">Discount Name</label>
                <div class="col-sm-9">
                  <input type="text" name="name" id="name" class="form-control"
                  data-msg-required="Name is required" value="@if(isset($discount)){{ $discount->name}}@endif" required/>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                  ...
                  data-msg-required="Description is required" value="@if(isset($discount)){{ $discount->discount_price }}@endif"  required/>
                  <label id="discount_price-server" class="server-error" for="amount"></label>
                </div>
              </div>
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-9">
                  <select class="form-control" name="status" >
                          <option  @if (isset($discount->status) && $discount->status == "1" ) selected="selected" @endif value="1">Active</option>
                          <option  @if (isset($discount->status) && $discount->status == "0" ) selected="selected" @endif value="0">Inactive</option>
                  </select>
                </div>
              </div>
